  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      <button
        ...
        className="absolute left-1/2 top-3 z-20 -translate-x-1/2 grid h-9 w-9 place-items-center rounded-full glass text-foreground/70 hover:text-foreground"
      >
      ...
      <button
        ...
        aria-label="Next"
        onClick={() => onActiveChange(clamp(activeIndex + 1))}
        className="absolute bottom-3 left-1/2 z-20 -translate-x-1/2 grid h-9 w-9 place-items-center rounded-full glass text-foreground/70 hover:text-foreground"
      >
...
function slideStyle(offset: number) {
  ...
  const z = 10 - abs;
  return { y, scale, opacity, zIndex: z };
}
